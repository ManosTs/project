...
public class RedBlackTreeVolume implements IRedBlackTreeVolume {
    ...
    private void printInorder(RBNode root) {
        if (root != leaf) {//if root is not leaf
            String color;
            if (root.color == RED) {//get red color of root if so
                color = "RED";
            } else {//get black color
                color = "BLACK";
            }

            printInorder(root.left);//first print the left nodes of tree
            System.out.println(root == this.root ? "ROOT--" + root.data + "(" + color + ")" : root.data + "(" + color + ")");//find the root and print it
            printInorder(root.right);//then print the right nodes of tree
        }
    }
